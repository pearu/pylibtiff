...
static PyObject *
bitarray_to_npyarray(bitarrayobject *self)
{
  PyObject *arr = NULL;
  npy_intp dims[] = {Py_SIZE(self)};
  arr = PyArray_ZEROS(1, dims, NPY_UBYTE, NPY_CARRAY);
  memcpy((void*)PyArray_DATA(arr), ((bitarrayobject *) self)->ob_item,  dims[0]);
  return arr;
}

PyDoc_STRVAR(to_npyarray_doc,
"toarray()\n\
\n\
Return a nympy array copy of the bitarray memory.");

static PyObject *
bitarray_toword(bitarrayobject *self, PyObject *args, PyObject *kwds)
{
  idx_t index = 0;
  unsigned char bits = 8;
  static char* kwlist[] = {"index", "bits", NULL};
  if (!PyArg_ParseTupleAndKeywords(args, kwds, "|ib:toword", kwlist,
				   &index, &bits))
    return NULL;
  if (bits<=8)
    {
      npy_uint8 x = *((npy_uint16*)((self)->ob_item + (index) / 8)) >> (index % 8);
      return Py_BuildValue("B",x & (NPY_MAX_UINT8>>(8-bits)));
    }
  if (bits<=16)
    {
      npy_uint16 x = (*((npy_uint32*)((self)->ob_item + (index) / 8))) >> (index % 8);
      return Py_BuildValue("H",x & (NPY_MAX_UINT16>>(16-bits)));
    }
#ifdef NPY_UINT64
  if (bits<=32)
    {
      npy_uint32 x = *((npy_uint64*)((self)->ob_item + (index) / 8)) >> (index % 8);
      return Py_BuildValue("I",x & (NPY_MAX_UINT32>>(32-bits)));
    }
#endif
#ifdef NPY_UINT128
  if (bits<=64)
    {
      npy_uint64 x = *((npy_uint128*)((self)->ob_item + (index) / 8)) >> (index % 8);
      return Py_BuildValue("K",x & (NPY_MAX_UINT64>>(64-bits)));
    }
  PyErr_SetString(PyExc_ValueError, "bits must be less or equal to 64");
#else
  PyErr_SetString(PyExc_ValueError, "bits must be less or equal to 32");
#endif
  return NULL;
}

PyDoc_STRVAR(toword_doc,
"toword(index=0, bits=8)\n\
\n\
...
bitarray_methods[] = {
    ...
    {"to01",         (PyCFunction) bitarray_to01,        METH_NOARGS,
     to01_doc},
    {"toarray",         (PyCFunction) bitarray_to_npyarray,        METH_NOARGS,
     to_npyarray_doc},
...
bitarray_new(PyTypeObject *type, PyObject *args, PyObject *kwds)
{
    ...
    if (PyArray_Check(initial))
      {
	int bytes = PyArray_NBYTES(initial);
	if (strcmp(endianStr, "<NOT_PROVIDED>")==0)
	  {
	    char c = PyArray_DESCR(initial)->byteorder;
	    if (c=='>')
	      endian = 1;
	    else if (c=='<')
	      endian = 0;
...
#error Cannot detect byte-order of numpy!
	...
	a = newbitarrayobject(type, bytes*NPY_BITSOF_CHAR, endian);
        if (a == NULL)
            return NULL;
        memcpy(((bitarrayobject *) a)->ob_item, (void*)PyArray_DATA(initial), bytes);
	return a;
      }
