...
__all__ = ['scaninfo', 'timestamps', 'eventlist', 'channelcolors',
           ...
           'channelfactors', 'channeldatatypes', 'lsmblock', 'lsminfo']
...
def IFDEntry_lsm_str_hook(entry):
    ...
    for name in entry.value.dtype.names:
        ...
        if func is not None:
            ...
            v = entry.value[name]
            lst.append('\n  %s=%s' % (name, v))
    return 'IFDEntry(tag=%s, value=%s(%s))' % (
        entry.tag_name, entry.type_name, ', '.join(lst))
...
def IFDEntry_lsm_init_hook(ifdentry):
    ...
    if ifdentry.tag == CZ_LSMInfo_tag:
        ...
        if CZ_LSMInfo_type not in tiff_module_dict['type2dtype']:
            ...
            tiff_module_dict['type2name'][CZ_LSMInfo_type] \
                = CZ_LSMInfo_type_name
            tiff_module_dict['type2bytes'][CZ_LSMInfo_type] \
                = CZ_LSMInfo_type_size
            tiff_module_dict['tag_name2value'][CZ_LSMInfo_type_name] \
                = CZ_LSMInfo_tag
        # assert ifdentry.count==CZ_LSMInfo_type_size,
        # `ifdentry.count,CZ_LSMInfo_type_size`
    ...
    if ifdentry.tag == CZ_LSMInfo_tag:
            ...
            if func is not None:
                ...
                if s is not None:
                    ...
                    if blockstart is None:
                        ...
                        blockstart, blockend = (
                            min(blockstart, start), max(blockend, end))
        ...
        if verbose_lsm_memory_usage:
            for offset in ifdentry.value['Reserved'][0]:
                if offset:
                    ifdentry.memory_usage.append(
                        (offset, offset, 'start of a unknown reserved field'))
        else:
...
def lsminfo(ifdentry, new_lsmblock_start=None):
    ...
    for name in ifdentry.value.dtype.names:
        ...
        if name.startswith('Offset') and value != 0:
            ...
            # assuming that unknown values in Reserved field are
            # offsets:
            for i in range(len(value)):
                if value[i] != 0:
                    if value[i] >= ifdentry.block_range[0] \
                       and value[i] <= ifdentry.block_range[1]:
                        # print 'chaning Reserved[%s] from %s to' %
                        # (i, value[i]),
                        value[i] += offset_diff
                        # print value[i]
                    else:
                        sys.stderr.write(
                            'Reserved[%s]=%s is out of block range %s.'
                            'lsminfo might not be correct.'
                            % (i, value[i], ifdentry.block_range))
        i += dt.itemsize
...
class ScanInfoEntry:
    ...
    def __init__(self, entry, type_name, label, data):
        ...
        if type_name == 'ASCII':
            ...
            self.type = 4
            self.header = numpy.array(
                [entry, 4, 4], dtype=numpy.uint32).view(dtype=numpy.ubyte)
        elif type_name == 'DOUBLE':
            self.type = 5
            self.header = numpy.array(
                [entry, 5, 8], dtype=numpy.uint32).view(dtype=numpy.ubyte)
        elif type_name == 'SUBBLOCK':
            self.type = 0
            self.header = numpy.array(
                [entry, 0, 0], dtype=numpy.uint32).view(dtype=numpy.ubyte)
        else:
    ...
    while 1:
        ...
        if type_name == 'ASCII':
            ...
            raise NotImplementedError(
                repr((type_name, hex(entry), type, size)))
...
class TimeStamps:
    ...
    def header(self):
        if self._header is None:
            offset = self.offset
            self._header = self.ifdentry.tiff.get_value(
                offset, CZ_LSMTimeStamps_header_dtype)
        return self._header
    ...
    def stamps(self):
        if self._stamps is None:
            ...
            self._stamps = self.ifdentry.tiff.get_values(
                self.offset + self.header.dtype.itemsize,
                numpy.float64, int(n))
        return self._stamps
    ...
    def toarray(self, target=None):
        ...
        if target is None:
            target = numpy.zeros((sz,), dtype=numpy.ubyte)
        dtype = target.dtype
        header = numpy.array([sz, self.stamps.size],
                             dtype=numpy.int32).view(dtype=dtype)
...
class EventEntry:
    ...
    def __str__(self):
        return 'EventEntry(time=%r, type=%r, description=%r)' % (
...
class EventList:
    ...
    def header(self):
        if self._header is None:
            self._header = self.ifdentry.tiff.get_value(
                self.offset, CZ_LSMEventList_header_dtype)
        return self._header
    ...
    def __str__(self):
        return '%s(events=[%s])' % (
            self.__class__.__name__, ','.join(map(str, self.events)))
    ...
    def toarray(self, target=None):
        ...
        if target is None:
            target = numpy.zeros((sz, ), dtype=numpy.ubyte)
        dtype = target.dtype
        header = numpy.array([sz, len(self.events)],
                             dtype=numpy.int32).view(dtype=dtype)
        target[:header.nbytes] = header
...
class ChannelColors:
    ...
    def header(self):
        if self._header is None:
            self._header = self.ifdentry.tiff.get_values(
                self.offset, numpy.int32, 10)
        return self._header
    ...
    def __str__(self):
        return '%s (names=%s, colors=%s)' % (
            self.__class__.__name__, self.names, self.colors)
    ...
    def toarray(self, target=None):
        ...
        if target is None:
            target = numpy.zeros((sz,), dtype=numpy.ubyte)
        dtype = target.dtype
        header = numpy.array([sz, len(self.colors), len(self.names),
                              0, 0, self.mono, 0, 0, 0, 0],
                             dtype=numpy.int32)
        names = ''
        for name in self.names:
...
class ChannelFactors:
    ...
    def __str__(self):
        return '%s(size=%r, offset=%r)' % (
            self.__class__.__name__, self.get_size(), self.offset)
...
class OffsetData:
    ...
    def __str__(self):
        return '%s(name=%r, size=%r, offset=%r)' % (
            self.__class__.__name__, self.offset_name,
...
def inputlut(ifdentry, debug=True):
    return lookuptable(
        ifdentry, 'OffsetInputLut', debug=debug)


def outputlut(ifdentry, debug=True):
    return lookuptable(
        ifdentry, 'OffsetOutputLut', debug=debug)


def vectoroverlay(ifdentry, debug=True):
    return drawingelement(
        ifdentry, 'OffsetVectorOverlay', debug=debug)


def roi(ifdentry, debug=True):
    return drawingelement(
        ifdentry, 'OffsetRoi', debug=debug)


def bleachroi(ifdentry, debug=True):
    return drawingelement(
        ifdentry, 'OffsetBleachRoi', debug=debug)


def meanofroisoverlay(ifdentry, debug=True):
    return drawingelement(
        ifdentry, 'OffsetMeanOfRoisOverlay', debug=debug)


def topoisolineoverlay(ifdentry, debug=True):
    return drawingelement(
        ifdentry, 'OffsetTopoIsolineOverlay', debug=debug)


def topoprofileoverlay(ifdentry, debug=True):
    return drawingelement(
        ifdentry, 'OffsetTopoProfileOverlay', debug=debug)


def linescanoverlay(ifdentry, debug=True):
    return drawingelement(
        ifdentry, 'OffsetLinescanOverlay', debug=debug)
...
CZ_LSMOffsetField_readers = dict(
    ...
    OffsetUnmixParameters=lambda ifdentry, debug=True: offsetdata(
        ifdentry, 'OffsetUnmixParameters', debug=debug),
    OffsetNextRecording=lambda ifdentry, debug=True: offsetdata(
        ifdentry, 'OffsetNextRecording', debug=debug),
    OffsetKsData=lambda ifdentry, debug=True: offsetdata(
        ifdentry, 'OffsetKsData', debug=debug),
...
CZ_LSMInfo_dtype_fields = [
    ...
    # 1: uint8, 2: uint12, 5: float32, 0: see OffsetChannelDataTypes:
    ('SDataType', numpy.int32),
